module.exports = app => {
  ...
  // 由于 sort((a, b) => Math.random() - 0.5) 并不能真正的随机打乱，所有需要Fisher–Yates 洗牌算法
  ...
  router.get('/news/init', async (req, res) => {
    ...
    const newsTitles = ["澜友风席卷快手，澜朋友带着他的优秀作者们来啦！", "海盐警方破获制售《王者荣耀》游戏外挂案件", "新年寄语丨让我们长风破浪，迎破晓之光", "《使命召唤手游》全新玩法-使命战场，限时开启，不可不试", "三款皮肤海报翻新【老亚瑟的答疑时间】", "1月6日全服不停机更新公告", "1月5日体验服停机更新公告", "1月3日体验服停机更新公告", "【重要公告】", "【重要公告】", "欢聚双旦福利不停，天狼运算者即将上架，奇迹圣诞限时返场", "恭喜DYG夺得KPL秋季赛冠军！冠军回馈来袭，双旦福利开启！", "参与活动免费解锁KPL限定皮肤个人专属购买6折特权", "【手Q独家】好友助力赢现金好礼", "【看KPL总决赛 赢豪华大礼】活动公告", "大浪淘沙始见金——王者荣耀全国大赛侧写", "2020冬冠选拔赛赛程公布，1月7日重庆QG对战南京Hero打响揭幕战！", "【战报】摘冠称王！恭喜虎牙XYG 4:1 企鹅秦 获得全国大赛总冠军！", "前瞻丨全国大赛冠军之争 昔日冠军守擂佛山之巅 破刃之师欲夺至高荣誉", "25日战报：虎牙XYG势如破竹，成都TG攻守兼备，全国大赛四强队伍诞生"]
    // 批量给newList对象赋值，slice: 方法可从已有的数组中返回选定的元素。
    // slice(0)相当于完全返回一个复制了cats的数组
    // sort：默认按字母顺序排序，字符串则由
  ...
  router.get('/news/list', async (req, res) => {
    // populate使用非常方便，但是缺点只能是一次性查找所有的信息，不能按分类查找。
    // const parent = await Category.findOne({
    //   name: '新闻分类'
    // }).populate({
    //   path: 'children',
    ...
    // aggregate 是 mongoose 里的聚合查询
    const cats = await Category.aggregate([
      // $match 条件查询，查询parent的_id，第一个parent为字段，第二个为上级分类
      {
        $match: {
          parent: parent._id
        }
      ...
      {
        $addFields: {
          // $slice 类似数组中的slice方法，第一个参数为$要查询的表，第二个参数为查询数量
          newsList: {
            $slice: ['$newsList', 5]
          }
        }
  ...
  router.get('/heroes/init', async (req, res) => {
    ...
    // rawData.filter(cat => cat.name !== '热门')
